...
4. **Türkiye ve global etki** (varsa) → bu pazarın gerçek karşılığı var, işte örnekler
5. **Çözüm yol haritası** → müşteri ne zaman ne alıyor
6. **Yatırım ve getiri** (varsa) → maliyet ve beklenen getiri yan yana, şeffaf
7. **Büyüme projeksiyonu** (varsa) → 3/6/12 ayda iş ne kadar büyüyor
8. **Riskler ve yönetimi** → ne biliniyor, nasıl kontrol ediliyor — endişe değil güven
9. **Teslim güvenilirliği** (varsa) → bu tarihe yetişir mi, tampon ne kadar
10. **Sonraki adım** → vizyonu ve sloganı tekrar bağla, kapanış
...
`business_case`, `competitive_analysis`, `market_impact`, `cost_estimate`, `growth_projection` ve `deadline`, `plan.json`'a girildiğinde otomatik birer slayt/görsel açar — girilmezse o slaytlar sessizce atlanır, boş bırakılmaz. Bunlar teknik olarak opsiyonel ama pitch'te varsayılan olarak beklenir (`references/discovery.md`).
