...
`business_case` (iş değeri), `competitive_analysis` (rakip başına fiyat + pazar konumu + güçlü/zayıf yanlar, en az 2-3 rakip + bizim farkımız), `swot` (güçlü/zayıf/fırsat/tehdit, dürüst), `traction` (gerçekleşmiş kanıt: pilot/LOI/bekleme listesi/alıntı), `market_impact` (Türkiye ve global pazar büyüklüğü + isimli örnekler), `deadline` (elinizdeki süre), `cost_estimate` (kurulum + aylık işletme gideri), `growth_projection` (3/6/12 ay birikmiş müşteri/gelir), `marketing_strategy` (kampanya fikirleri + renk paleti + güncel AI içerik araçları) ve `slogan` (kapak için, birden fazla öneriyle) teknik olarak opsiyoneldir ama pitch deck'te **varsayılan olarak beklenir** — "opsiyonel" atlama izni değildir, sadece gerçekten imkansızsa boş kalır. Doldurulursa sunumda ve `roadmap.md`'de otomatik birer bölüm/slayt/görsel açar. Hammaddesi WebSearch ile yapılan gerçek pazar/rakip/AI-araç araştırması ve discovery'deki "iş fikrini derinlemesine analiz et", "talebi analiz et", "rakip analizi ve işletme gideri", "SWOT analizi", "Ekip ve Traction", "Türkiye ve global etki", "büyüme projeksiyonu", "pazarlama ve reklam stratejisi" ve "slogan" adımlarıdır — `references/discovery.md`. Uydurma rakam/rakip/örnek şirket/AI araç adı yazma; bulamazsan "bilinmiyor, doğrulanmalı" de. `marketing_strategy.ai_tools` özellikle her üretimde yeniden WebSearch ile doğrulanmalı — hızla eskir.

`team_members` bu listenin **istisnasıdır** — varsayılan değildir, SADECE kullanıcının verdiği gerçek isimler varsa doldurulur, asla uydurulmaz. Doldurulmadığında "Ekip" slaydı boş kalmaz: `plan_capacity.py`'nin toplam effort/kritik yoldan otomatik hesapladığı, isim gerektirmeyen bir ekip büyüklüğü önerisi (1-3/3-5/5+ kişi karşılaştırması) devreye girer — bu her zaman üretilir.

`success_metric.measured_at` gibi dahili faz referanslarını ve `deadline.driver` dışındaki yerlerde deadline'ı bir KPI kartı olarak sunuma koyma — `references/deck.md`'deki "KPI kartlarında dahili zamanlama/program referansı kullanma" kuralına bak. Faz başına ayrı slayt da yok — `journey.svg` tüm fazları tek slaytta gösterir, tekrar etme.
...
`cost_estimate` doluysa **işletme gideri** toplamlarını (kurulum, aylık, yıllık), `growth_projection` doluysa **büyüme** dönemleri arası net artışı da hesaplar — kalemleri elle toplama.
...
Beş temel SVG üretir: `journey` (faz yolculuğu), `timeline` (sprint zaman şeridi), `depgraph` (bağımlılık + kritik yol), `riskmatrix` (etki × olasılık), `capacity` (paralellik ve agent). `plan.json`'da `deadline` doluysa `deadline.svg` (gauge), `cost_estimate` doluysa `cost.svg` (**pasta grafik**), `growth_projection` doluysa `growth.svg` (**sütun grafik**, çift panel), `market_impact` doluysa `market_impact.svg` (**iki panelli kart**), `swot` doluysa `swot.svg` (**2x2 renkli panel**), `traction` doluysa `traction.svg` (**stat + alıntı**), `marketing_strategy` doluysa `marketing.svg` (**üç kolon**) ve `ad_creative.svg` (**örnek reklam mockup'ı**, palet varsa) eklenir; doldurulmayan alanlar sessizce atlanır. `team_size.svg` (**ekip büyüklüğü önerisi**, isim gerektirmez) her zaman üretilir; `team_members` gerçek isimlerle doldurulduysa `team.svg` (**kart dizisi**) ayrıca üretilir. Grafik türleri kasıtlı çeşitlendirilmiştir — hepsi çubuk grafik olmasın. `--audience delivery` aynı plandan teknik adlarla ikinci bir set üretir.
